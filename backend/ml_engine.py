...
1. Bayesian conditional probabilities (from training data)
2. ML model feature importance (from XGBoost)
3. Regional environmental data (from database)
...
try:
    ...
    models = None
    print("⚠️  Models not imported - ml_engine.py running standalone")
...
class BayesianRiskAnalyzer:
    ...
    def __init__(self, models_dir: str = "./models"):
        ...
        if self.model and self.bayesian_probs:
            ...
            print("⚠️  Bayesian Risk Analyzer initialized with missing components")
        print("=" * 80)
    ...
    def _load_bayesian_probabilities(self):
        ...
        try:
            ...
            if bayesian_path.exists():
                ...
                print(f"   Base risk: {self.bayesian_probs['metadata']['p_high_risk']:.2%}")
            else:
    ...
    def get_regional_drivers(
        ...
        try:
            ...
            # Get top 3 drivers
            top_drivers = sorted(risk_drivers, key=lambda x: x['hybrid_score'], reverse=True)[:3]
    ...
    def _calculate_hybrid_scores(self, regional_data: Dict) -> List[Dict]:
        ...
        for feature, stats in regional_data.items():
            ...
            # Weights: Bayesian 40%, ML Importance 30%, Deviation 30%
            hybrid_score = (
                0.4 * bayesian_score +
                0.3 * ml_importance +
                0.3 * deviation_score
            ...
            risk_drivers.append({
                ...
                'bayesian_probability': float(round(bayesian_score * 100, 1)),  # e.g., 85.3%
                'bayesian_label': f"P(High Risk | {get_friendly_name(feature)} elevated)",
                'bayesian_score': float(round(bayesian_score, 4)),  # Keep original for compatibility
                
                # Model Feature Importance (clearly labeled)
                'model_importance': float(round(ml_importance * 100, 2)),  # e.g., 20.68%
                'model_label': "XGBoost Feature Weight",
                'ml_importance': float(round(ml_importance, 4)),  # Keep original for compatibility
                
                # Combined/Hybrid Score (clearly labeled)
                'hybrid_score': float(round(hybrid_score, 4)),
                'hybrid_score_percentage': float(round(hybrid_score * 100, 2)),  # e.g., 175.78%
                'hybrid_label': "Combined Risk Driver Score",
                
                # Deviation Score
                'deviation_score': float(round(deviation_score, 4)),
                'deviation_percentage': float(round(deviation_score * 100, 1)),  # e.g., 85.0%
                'deviation_direction': deviation_direction,
                
                # Values
                'current_value': float(round(current_value, 2)),
                'current_value_display': f"{round(current_value, 2)} {unit}".strip(),
                'safe_value': safe_value,
                'safe_value_display': safe_value_display,
                'deviation': f"{round(deviation_score * 100, 1)}% {deviation_direction} threshold",
                ...
                'std_dev': float(round(stats['std'], 2)),
    ...
    def _get_bayesian_score(
        ...
        """Get Bayesian probability score (0-1)"""
    ...
    def _get_ml_importance(self, feature: str) -> float:
        """Get normalized ML feature importance (0-1)"""
    ...
    def _get_deviation_score(
        ...
        """Calculate deviation from safe threshold (0-1)"""
        ...
        # Handle range-based thresholds (pH, temperature, humidity)
            ...
            if safe_min <= value <= safe_max:
                ...
                if value < safe_min:
                    ...
                    max_deviation = safe_min - threshold.get('elevated_min', safe_min - 5)
                else:
                    deviation = value - safe_max
                    max_deviation = threshold.get('elevated_max', safe_max + 5) - safe_max
                
                return min(deviation / max_deviation, 1.0) if max_deviation > 0 else 0.0
    ...
    def _determine_risk_level(
        ...
        # Handle range-based thresholds
            ...
            if threshold['safe_min'] <= value <= threshold['safe_max']:
                return 'safe'
            elif threshold.get('elevated_min', 0) <= value <= threshold.get('elevated_max', 100):
                return 'elevated'
            else:
                return 'high_risk'
    ...
    def _get_recommendation(self, driver: Dict) -> str:
        ...
        recommendations = {
            'fecal_coliform_per_100ml': {
                'high_risk': f"CRITICAL: Fecal coliform at {current:.0f} per 100ml (safe: {safe}). Immediate water treatment and source investigation required. Do not consume without boiling.",
                'elevated': f"WARNING: Elevated fecal coliform at {current:.0f} per 100ml. Implement water treatment and monitor source.",
                'safe': "Fecal coliform within safe limits. Continue monitoring."
            },
            'ph': {
                'high_risk': f"pH at {current:.1f} is outside safe range ({safe}). Adjust water treatment process. Extreme pH can affect disinfection effectiveness.",
                'elevated': f"pH at {current:.1f} is slightly outside optimal range. Monitor and adjust if needed.",
                'safe': "pH within safe range."
            },
            'turbidity_ntu': {
                'high_risk': f"High turbidity ({current:.1f} NTU, safe: <{safe}). Improve filtration. High turbidity shields pathogens from disinfection.",
                'elevated': f"Elevated turbidity ({current:.1f} NTU). Enhance filtration processes.",
            ...
            'nitrate_mg_l': {
                'high_risk': f"High nitrate ({current:.1f} mg/l, safe: <{safe}). Risk of methemoglobinemia in infants. Identify agricultural/sewage contamination source.",
                'elevated': f"Elevated nitrate ({current:.1f} mg/l). Monitor for agricultural runoff.",
...
def test_analyzer():
    ...
    if risk_analyzer.feature_importance is not None:
        print(f"\n📊 Top 5 ML Features by Importance:")
        for i, row in risk_analyzer.feature_importance.head(5).iterrows():
            print(f"   {i+1}. {row['feature']}: {row['importance']:.4f}")
    
    if risk_analyzer.bayesian_probs:
        print(f"\n📊 Bayesian Probabilities:")
        print(f"   Base risk: {risk_analyzer.bayesian_probs['metadata']['p_high_risk']:.2%}")
