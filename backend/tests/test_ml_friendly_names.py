...
def test_friendly_names():
    ...
    sample_driver = {
        'feature': 'fecal_coliform_per_100ml',
        'feature_display': get_friendly_name('fecal_coliform_per_100ml'),
        'factor': get_friendly_name('fecal_coliform_per_100ml'),
        'bayesian_probability': 85.3,
        'bayesian_label': 'P(High Risk | fecal coliform elevated)',
        'model_importance': 20.68,
        'model_label': 'XGBoost Feature Weight',
        'hybrid_score_percentage': 175.78,
        'current_value_display': '2500 per 100ml',
        'safe_value_display': '<500 per 100ml'
    }
    ...
    print("=" * 80)
    print("\n💡 Next Steps:")
    print("   1. Restart backend server: python backend/start_server.py")
    print("   2. Test API endpoint: GET /regional-risk/{region}/drivers")
    print("   3. Verify friendly names appear in modal")
