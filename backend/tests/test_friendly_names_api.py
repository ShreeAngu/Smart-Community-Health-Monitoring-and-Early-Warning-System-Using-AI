            ...
            if response.status_code == 200:
                ...
                if drivers:
                    ...
                    if '_' in feature_display:
                        print(f"\n   ⚠️  WARNING: Feature display still contains underscores!")
                    else:
                    ...
                    if '_' in bayesian_label and 'P(High Risk' in bayesian_label:
                        print(f"   ⚠️  WARNING: Bayesian label contains underscores!")
                    else:
                        print(f"   ✅ Bayesian label is human-readable")
                else:
                    print(f"   ℹ️  No drivers found (insufficient data)")
                    
            elif response.status_code == 404:
                print(f"⚠️  Status: {response.status_code} - No data for region")
            else:
                print(f"❌ Status: {response.status_code}")
