    ...
    for method, endpoint, description in allowed_endpoints:
        try:
            if method == "GET":
                ...
                success = response.status_code in [200, 404]  # 404 if no data exists
                status = "✓ PASS" if success else "✗ FAIL"
                print(f"{status} {method} {endpoint} - {description} (Status: {response.status_code})")
                results.append(success)
            elif method == "POST":
                # Just verify the endpoint is accessible (will fail without proper data, but shouldn't be 403)
...
def test_invalid_token():
    ...
    for token, description in invalid_tokens:
        try:
            ...
            status = "✓ PASS" if success else "✗ FAIL"
            print(f"{status} {description} - Expected 401, Got {response.status_code}")
            results.append(success)
...
def test_no_token():
    ...
    try:
        ...
        success = response.status_code == 401
        status = "✓ PASS" if success else "✗ FAIL"
        print(f"{status} Access without token - Expected 401, Got {response.status_code}")
        return success
    except Exception as e:
...
def test_token_expiration():
    ...
    print("Current token expiry is set in auth.py (default: 30 minutes)")
    print("⊘ SKIP - Manual test recommended for token expiration")
    print("\nTo test manually:")
    print("1. Set ACCESS_TOKEN_EXPIRE_MINUTES to 1 in auth.py")
    print("2. Login and get a token")
    print("3. Wait 2 minutes")
    print("4. Try to access protected endpoint")
    print("5. Should receive 401 Unauthorized")
...
def main():
    ...
    # Test 1: Login and role verification
    ...
    # Test 3: Community user restrictions
    if community_token:
    ...
    # Test 5: No token access
    no_token_success = test_no_token()
    all_results.append(no_token_success)
    
    # Test 6: Token expiration (skipped in automated tests)
