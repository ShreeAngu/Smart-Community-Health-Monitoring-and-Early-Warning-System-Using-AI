...
print(f"\n📊 Total reports in last 7 days: {len(recent_reports)}")

# Group by region
region_counts = {}
for report in recent_reports:
    region_counts[report.region] = region_counts.get(report.region, 0) + 1
...
print(f"\n🔮 Total predictions in last 7 days: {len(recent_predictions)}")
...
for region, scores in pred_by_region.items():
    ...
    print(f"   {region}: {len(scores)} predictions, avg={avg_score:.1f}, high-risk={high_risk}")
...
if coimbatore_preds:
    print("\nRecent predictions:")
    for i, pred in enumerate(coimbatore_preds[-5:], 1):
        print(f"  {i}. Risk={pred.risk_level} ({pred.risk_score:.1f}%), "
