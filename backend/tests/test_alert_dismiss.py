...
def test_alert_dismiss():
    ...
    # Step 2: Get current alerts
    print("2️⃣ Fetching current alerts...")
    ...
    if len(alerts) == 0:
        print("\n⚠️  No alerts found. Creating a test alert first...")
    ...
    # Step 3: Test dismissing an alert
    if len(alerts) > 0:
        test_alert_id = alerts[0]['id']
        print(f"\n3️⃣ Testing DISMISS for alert ID {test_alert_id}...")
    ...
    # Step 4: Test resolving an alert
    if len(alerts) > 1:
        test_alert_id = alerts[1]['id']
        print(f"\n4️⃣ Testing RESOLVE for alert ID {test_alert_id}...")
    ...
    # Step 5: Verify updated alerts
    print("\n5️⃣ Fetching updated alerts...")
    updated_alerts_response = requests.get(f"{BASE_URL}/alerts", headers=headers)
    updated_alerts = updated_alerts_response.json()
    
    print(f"\n📊 Alert Status Summary:")
    active = sum(1 for a in updated_alerts if a.get('status', 'active') == 'active')
    resolved = sum(1 for a in updated_alerts if a.get('status') == 'resolved')
    dismissed = sum(1 for a in updated_alerts if a.get('status') == 'dismissed')
