...
def append_report_to_csv(report_data: Dict[str, Any], prediction_data: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        if prediction_data is None or 'error' in prediction_data:
            print(f"⚠️ CSV sync skipped: Prediction failed or unavailable")
            return False
        ...
        # Build new row matching the CSV structure (40 columns)
        ...
        new_row = {
            ...
            'population_density': safe_int(report_data.get('population_density'), 1000),
            'age': safe_int(report_data.get('age'), 30),
            ...
            'water_quality_index': safe_float(water_metrics.get('water_quality_index'), 50.0),
            'ph': safe_float(water_metrics.get('ph'), 7.0),
            'turbidity_ntu': safe_float(water_metrics.get('turbidity_ntu'), 5.0),
            'dissolved_oxygen_mg_l': safe_float(water_metrics.get('dissolved_oxygen_mg_l'), 8.0),
            'bod_mg_l': safe_float(water_metrics.get('bod_mg_l'), 3.0),
            'fecal_coliform_per_100ml': safe_int(water_metrics.get('fecal_coliform_per_100ml'), 0),
            'total_coliform_per_100ml': safe_int(water_metrics.get('total_coliform_per_100ml'), 0),
            'tds_mg_l': safe_float(water_metrics.get('tds_mg_l'), 300.0),
            'nitrate_mg_l': safe_float(water_metrics.get('nitrate_mg_l'), 5.0),
            'fluoride_mg_l': safe_float(water_metrics.get('fluoride_mg_l'), 0.5),
            'arsenic_ug_l': safe_float(water_metrics.get('arsenic_ug_l'), 5.0),
            'open_defecation_rate': safe_float(report_data.get('open_defecation_rate'), 0.1),
            'toilet_access': safe_int(report_data.get('toilet_access'), 1),
            'sewage_treatment_pct': safe_float(report_data.get('sewage_treatment_pct'), 50.0),
            ...
            'avg_temperature_c': safe_float(report_data.get('avg_temperature_c'), 25.0),
            'avg_rainfall_mm': safe_float(report_data.get('avg_rainfall_mm'), 100.0),
            'avg_humidity_pct': safe_float(report_data.get('avg_humidity_pct'), 60.0),
