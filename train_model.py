...
# Label Encode binary features (already 0/1, but ensuring consistency)
label_encode_cols = ['is_urban', 'flooding']
print(f"Label Encoding (binary): {label_encode_cols}")
# These are already 0/1 from the data exploration, so no action needed
...
print(f"XGBoost - Accuracy: {xgb_accuracy:.4f}, F1-Score: {xgb_f1:.4f}, ROC-AUC: {xgb_roc_auc:.4f}")
...
print(f"Random Forest - Accuracy: {rf_accuracy:.4f}, F1-Score: {rf_f1:.4f}, ROC-AUC: {rf_roc_auc:.4f}")
